...
namespace Froststrap
{
    public class Bootstrapper
    {
        ...
        public Bootstrapper(LaunchMode launchMode)
        {
            ...
            // Eclipse: force Player onto LIVE/production (MrEx channel lock)
            if (!IsStudioLaunch && App.Settings.Prop.ForceLiveChannel)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private static bool TryResolvePlayerVersionPin(out string pinnedGuid)
        {
            pinnedGuid = "";
            ...
            else if (!string.IsNullOrEmpty(App.Settings.Prop.ActiveVersionProfileId))
            {
                profile = App.Settings.Prop.VersionProfiles
                    .FirstOrDefault(p => p.Id == App.Settings.Prop.ActiveVersionProfileId);
            }

            if (profile is not null && Utility.VersionGuidValidator.IsWellFormed(profile.VersionGuid))
            {
            ...
            {
                pinnedGuid = App.Settings.Prop.CustomVersionGuid;
                return true;
            }
