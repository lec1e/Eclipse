...
namespace Froststrap.UI.ViewModels.Settings
{
    public partial class AccountSelectorViewModel : NotifyPropertyChangedViewModel, IDisposable
    {
        ...
        private AltManAccount? _currentAccount;
        public AltManAccount? CurrentAccount
        {
            get => _currentAccount;
        ...
        {
            var invalidAccounts = new List<AltManAccount>();

            foreach (var account in _accountManager.Accounts.ToList())
            {
        ...
        private async Task LoadDataAsync()
        {
            try
            {
                ...
                var accountIds = mgr.Accounts.Select(acc => acc.UserIdLong).Where(id => id > 0).ToList();
                ...
                {
                    CurrentAccountAvatarUrl = GetAccountAvatarUrl(CurrentAccount.UserIdLong);
                }
                ...
                {
                    var url = _accountAvatarUrls.TryGetValue(account.UserIdLong, out var u) ? u : null;
        ...
        private void OnActiveAccountChanged(AltManAccount? account)
        {
            Dispatcher.UIThread.Post(() =>
            {
                CurrentAccount = account;
                CurrentAccountAvatarUrl = account != null ? GetAccountAvatarUrl(account.UserIdLong) : null;
                OnPropertyChanged(nameof(CurrentAccountDisplayName));
        ...
        private async Task RefreshPresenceAsync()
        {
            ...
            try
            {
                var presence = await AccountManager.GetUserPresenceAsync(CurrentAccount.UserIdLong);
                if (presence != null)
                {
        ...
        private async Task AddAccount()
        {
            ...
            try
            {
                ...
                if (await HandleDuplicateAccountAsync(newAccount.UserId))
                    return;
                ...
                var stored = _accountManager.Accounts.FirstOrDefault(a => a.UserIdLong == newAccount.UserId);
                if (stored is not null && Accounts.All(a => a.UserId != stored.UserIdLong))
                    Accounts.Add(new AccountWithAvatar(stored, url));
        ...
        private async Task<bool> HandleDuplicateAccountAsync(long userId)
        {
            var existing = _accountManager.Accounts.FirstOrDefault(a => a.UserIdLong == userId);
            if (existing != null)
            {
                _accountManager.SetActiveAccount(existing.UserIdLong);
                IsDropdownOpen = false;
        ...
        {
            if (await HandleDuplicateAccountAsync(account.UserId))
                return;
            ...
            var stored = _accountManager.Accounts.FirstOrDefault(a => a.UserIdLong == account.UserId);
            if (stored is not null && Accounts.All(a => a.UserId != account.UserId))
            {
                Accounts.Add(new AccountWithAvatar(stored, avatarUrl));
            }
        ...
        public class AccountWithAvatar(AltManAccount account, string? avatarUrl)
        {
            public AltManAccount Account { get; } = account;
            public string? AvatarUrl { get; } = avatarUrl;

            public string Username => Account.Username;
            public string DisplayName => Account.DisplayName;
            public long UserId => Account.UserIdLong;
        }

        public string? GetAccountAvatarUrl(long userId)
            => _accountAvatarUrls.TryGetValue(userId, out var url) ? url : null;
