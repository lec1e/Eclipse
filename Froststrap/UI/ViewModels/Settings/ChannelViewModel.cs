...
namespace Froststrap.UI.ViewModels.Settings
{
    public class ChannelViewModel : NotifyPropertyChangedViewModel
    {
        ...
        // Pre-release auto-updates removed — Eclipse only tracks stable releases.
        public bool PreReleaseUpdatesEnabled
        {
            get => false;
            set
            {
                // Force stable whenever something still tries to enable pre-release.
                if (SelectedUpdateCheck is UpdateCheck.Test or UpdateCheck.Both)
                    SelectedUpdateCheck = UpdateCheck.Stable;
        ...
        public UpdateCheck SelectedUpdateCheck
        {
            get
            {
                // Eclipse never auto-updates to pre-release builds.
                var v = App.Settings.Prop.UpdateChecks;
                if (v is UpdateCheck.Test or UpdateCheck.Both)
                {
                    App.Settings.Prop.UpdateChecks = UpdateCheck.Stable;
                    return UpdateCheck.Stable;
                }
                return v;
            }
            set
            {
                if (value is UpdateCheck.Test or UpdateCheck.Both)
                    value = UpdateCheck.Stable;
